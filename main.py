## @package main
#  @brief A script that uses a webcam to detect if a person is slouching and provides 
# visual and audio warnings, while also displaying historical data in real-time.
...
## @brief Update the plot with the latest data from the CSV file.
def update_plot():
...
## @brief Sort landmarks by their z values.
#  @param landmarks List of landmarks to be sorted.
#  @return Sorted list of landmarks.
def sort_landmarks_by_z(landmarks):
    return sorted(landmarks, key=lambda landmark: landmark.z)
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            ## Sort landmarks by their z values
            sorted_landmarks = sort_landmarks_by_z(relevant_landmarks)

            ## Use sorted landmarks for further calculations
            left_shoulder_z = sorted_landmarks[0].z
            right_shoulder_z = sorted_landmarks[1].z
            left_hip_z = sorted_landmarks[2].z
            right_hip_z = sorted_landmarks         ## Sort landmarks by their z values
            sorted_landmarks = sort_landmarks_by_z(relevant_landmarks)
            ...
            right_shoulder_z = sorted_landmarks[1].z
            left_hip_z = sorted_landmarks[2].z
            right_hip_z = sorted_landmarks[3].z
            mid_hip_z = (left_hip_z + right_hip_z) / 2
            left_elbow_z = sorted_landmarks[4].z
            right_elbow_z = sorted_landmarks[5].z
            mid_hip_z = (left_hip_z + right_hip_z) / 2
            left_elbow_z = sorted_landmarks[4].z
            right_elbow_z = sorted_landmarks[5].z
